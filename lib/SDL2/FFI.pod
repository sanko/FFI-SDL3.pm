...
SDL2::FFI is an FFI::Platypus-backed wrapper around the B<S>imple
...
up to an eventual 1.0 release. This package is named C<SDL2::FFI> because
C<SDL2> is being squatted on and I'd rather make games than play them over
namespaces.
...
=item C<flags> which may be any be imported with the L<< C<:init>|SDL2::FFI/C<:init> >> tag and may be OR'd together.
...
The category to assign a priority to. These may be improted with the L<<
...
Return value is a L<SDL2::FFI::Version> object.
...
The numeric ID is what L<SDL::WindowEvent> references, and is necessary to map
these events to specific L<SDL::Window> objects.
...
=item C<icon> - an L<SDL2::Surface> structure containig the icon for the window
...
... )> >> is intended to be write-only; it will notguarantee the previous
...
=item C<rect> - an L<SDL::Rect> structure representing the rectangle to draw
...
They may be imported with the C<:audio> tag.
...
These are the flags which may be passed to L<< C<SDL_Init( ... )>|/C<SDL_Init(
...
=item C<SDL_WINDOWEVENT_MOVED> - Window has been moved to data1, data2

=item C<SDL_WINDOWEVENT_RESIZED> - Window has been resized to data1xdata2
