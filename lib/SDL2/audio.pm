...
These functions may be imported with the C<:audio> tag.
...
Returns an L<< SDL_AudioStatus|/C<:audioStatus> >>.
...
This function is a compatibility convienience function that wraps L<<
...
You can choose to avoid callbacks and use L<< C<SDL_QueueAudio( ...
)>|/C<SDL_QueueAudio( ... )> >> instead, if you like. Just open your audio
device with a NULL callback.
...
Defines and Enumerations listed here may be imported from SDL::FFI with the
following tags:
...
=item C<SDL_MIX_MAXVOLUME>

=item C<SDL_AUDIOCVT_MAX_FILTERS>

Upper limit of filters in SDL_AudioCVT

The maximum number of C<SDL_AudioFilter> functions in L<SDL2::AudioCVT> is
currently limited to C<9>. The C<< SDL2::AudioCVT-E<gt>filters( ) >> array has
10 pointers, one of which is the terminating NULL pointer.
...
Native audio byte ordering (based on endianness of system).
