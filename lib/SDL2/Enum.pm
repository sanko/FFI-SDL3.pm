package SDL2::Enum {
    ...
    define SDL_KeyCode => [
        ...
        [ SDLK_DELETE     => ord "\x7F" ],
        [ SDLK_END        => sub () { SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_END() ) } ],
        [ SDLK_PAGEDOWN   => sub () { SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_PAGEDOWN() ) } ],
        [ SDLK_RIGHT      => sub () { SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_RIGHT() ) } ],
        [ SDLK_LEFT       => sub () { SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_LEFT() ) } ],
        [ SDLK_DOWN       => sub () { SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_DOWN() ) } ],
        [ SDLK_UP         => sub () { SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_UP() ) } ],
        [   SDLK_NUMLOCKCLEAR =>
                sub () { ord SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_NUMLOCKCLEAR() ) }
        ],
        [ SDLK_KP_DIVIDE => sub () { SDL_SCANCODE_TO_KEYCODE( SDL_SCANCODE_KP_DIVIDE() ) } ],
    ...
    define RWops => [
        [ SDL_RWOPS_UNKNOWN => 0 ], [ SDL_RWOPS_WINFILE => 1 ], [ SDL_RWOPS_STDFILE   => 2 ],
        [ SDL_RWOPS_JNIFILE => 3 ], [ SDL_RWOPS_MEMORY  => 4 ], [ SDL_RWOPS_MEMORY_RO => 5 ],
        ...
        #endif
        RWseek => [ [ RW_SEEK_SET => 0 ], [ RW_SEEK_CUR => 1 ], [ RW_SEEK_END => 2 ] ];
    ...
    enum SDL_Scancode => [
        ...
        [ SDL_SCANCODE_MODE           => 257 ], [ SDL_SCANCODE_AUDIONEXT      => 258 ],
        [ SDL_SCANCODE_AUDIOPREV      => 259 ], [ SDL_SCANCODE_AUDIOSTOP      => 260 ],
        [ SDL_SCANCODE_AUDIOPLAY      => 261 ], [ SDL_SCANCODE_AUDIOMUTE      => 262 ],
        [ SDL_SCANCODE_MEDIASELECT    => 263 ], [ SDL_SCANCODE_WWW            => 264 ],
        [ SDL_SCANCODE_MAIL           => 265 ], [ SDL_SCANCODE_CALCULATOR     => 266 ],
        ...
        [ SDL_SCANCODE_APP2             => 284 ], [ SDL_SCANCODE_AUDIOREWIND => 285 ],
        [ SDL_SCANCODE_AUDIOFASTFORWARD => 286 ], [ SDL_NUM_SCANCODES        => 512 ],
    ];
    enum SDL_SensorType => [
    ...
    # These are mixed case in SDL_shape.h but FFI::Platypus hates mixed case enums?
    define windowShapeMode => [
        [ ShapeModeDefault              => 0 ],
    ...
    enum
        ...
        qw[SDL_TOUCH_DEVICE_DIRECT SDL_TOUCH_DEVICE_INDIRECT_ABSOLUTE SDL_TOUCH_DEVICE_INDIRECT_RELATIVE]
        ];
...
=item C<SDL_RWOPS_VITAFILE> - Vita file
...
Values of this type are used to represent keyboard keys, among other places in
the L<SDL2::Keysym>->scancode() field of the L<SDL2::Event> structure.

The values in this enumeration are based on the USB usage page standard:
L<https://www.usb.org/sites/default/files/documents/hut1_12v2.pdf>
...
=item C<SDL_SCANCODE_BACKSLASH> - Located at the lower left of the return key on ISO keyboards and at the right end of the QWERTY row on ANSI keyboards.
...
Windows layout, DOLLAR SIGN and POUND SIGN in a Swiss German layout, NUMBER
SIGN and APOSTROPHE in a German layout, GRAVE ACCENT and POUND SIGN in a French
Mac layout, and ASTERISK and MICRO SIGN in a French Windows layout.

=item C<SDL_SCANCODE_NONUSHASH> - ISO USB keyboards actually use this code
...
=item C<SDL_SCANCODE_GRAVE> - Located in the top left corner (on both ANSI
...
=item C<SDL_SCANCODE_INSERT> - insert on PC, help on some Mac keyboards (but does send code 73, not 117)
...
=item C<SDL_SCANCODE_NUMLOCKCLEAR> -num lock on PC, clear on Mac keyboards
...
=item C<SDL_SCANCODE_NONUSBACKSLASH> - This is the additional key that ISO
...
=item C<SDL_SCANCODE_APPLICATION> - windows contextual menu, compose

=item C<SDL_SCANCODE_POWER> - The USB document says this is a status flag,
...
=item C<SDL_SCANCODE_AGAIN> - redo
...
=item C<SDL_SCANCODE_INTERNATIONAL1> - used on Asian keyboards, see footnotes in USB doc

=item C<SDL_SCANCODE_INTERNATIONAL2>

=item C<SDL_SCANCODE_INTERNATIONAL3> - Yen
...
=item C<SDL_SCANCODE_MODE> - I'm not sure if this is really not covered
...
Usage page C<0x0C> (additional media keys)
...
=item C<SDL_NUM_SCANCODES> - not a key, just marks the number of scancodes for array bounds
...
Basic boolean values for added sugar.
...
=item C<SDL_MIX_MAXVOLUME> - Maximum volume
...
autocenter encodings artesian buttonstate binarized scancodes trackpad OSes
RWops -num gr
