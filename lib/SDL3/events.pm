package SDL3::events 0.01 {
    ...
    use SDL3::mouse;
    use SDL3::joystick;
    use SDL3::gamecontroller;
    use SDL3::quit;
    use SDL3::gesture;
    use SDL3::touch;
    #
    use SDL3::version;    # Not in upstream header but required for SDL_version
    use SDL3::syswm;      # Header is not included by default but let's make life easy on folks
    ...
    package SDL3::TextEditingEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
            windowId  => 'uint32',
            _text     => 'char[' . SDL3::SDL_TEXTEDITINGEVENT_TEXT_SIZE() . ']',
            start     => 'sint32',
            length    => 'sint32';
    ...
    package SDL3::TextInputEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
            windowId  => 'uint32',
            _text     => 'char[' . SDL3::SDL_TEXTINPUTEVENT_TEXT_SIZE() . ']';
    ...
    package SDL3::ControllerAxisEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
    ...
    package SDL3::ControllerButtonEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
    ...
    package SDL3::ControllerDeviceEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
    ...
    package SDL3::ControllerTouchpadEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
    ...
    package SDL3::ControllerSensorEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
    ...
    package SDL3::MultiGestureEvent {
        use SDL3::Utils;
        our $TYPE = has
            type       => 'uint32',
            timestamp  => 'uint32',
    ...
    package SDL3::DollarGestureEvent {
        use SDL3::Utils;
    ...
    package SDL3::QuitEvent {
        use SDL3::Utils;
        our $TYPE = has type => 'uint32', timestamp => 'uint32';
    };

    package SDL3::OSEvent {
        use SDL3::Utils;
        our $TYPE = has type => 'uint32', timestamp => 'uint32';
    };

    package SDL3::UserEvent {
        use SDL3::Utils;
        our $TYPE = has
            type      => 'uint32',
            timestamp => 'uint32',
    ...
    attach events => {
        ...
        SDL_HasEvent =>
            [ ['uint32'] => 'bool' => sub ( $inner, @etc ) { SDL3::SDL_Yield(); $inner->(@etc) } ],
        SDL_HasEvents => [
            [ 'uint32', 'uint32' ] => 'bool' =>
                sub ( $inner, @etc ) { SDL3::SDL_Yield(); $inner->(@etc) }
        ],
        SDL_FlushEvent =>
            [ ['uint32'] => sub ( $inner, @etc ) { SDL3::SDL_Yield(); $inner->(@etc) } ],
        SDL_FlushEvents => [
            [ 'uint32', 'uint32' ] => sub ( $inner, @etc ) { SDL3::SDL_Yield(); $inner->(@etc) }
        ],
        SDL_PollEvent => [
            ['SDL_Event'] => 'int' => sub ( $inner, @etc ) { SDL3::SDL_Yield(); $inner->(@etc) }
        ],
        SDL_WaitEvent => [
            ['SDL_Event'] => 'int' => sub ( $inner, @etc ) { SDL3::SDL_Yield(); $inner->(@etc) }
        ],
    ...
    define events => [
        [ SDL_GetEventState => sub ($type) { SDL3::SDL_EventState( $type, SDL3::SDL_QUERY() ) } ] ];
...
SDL3::events - SDL Event Handling
...
SDL3::events represents the library's version as three levels: major, minor,
and patch level.
...
stored in the L<SDL3::Event> structure pointed to by C<event>. The C<1>
...
=item C<event> - the L<SDL3::Event> structure to be filled with the next event from the queue, or C<undef>
...
If C<event> is not C<undef>, the next event is removed from the queue and
stored in the L<SDL3::Event> structure pointed to by C<event>.
...
=item C<event> - the L<SDL3::Event> structure to be filled in with the next event from the queue, or C<undef>
...
=item C<event> - the L<SDL3::Event> to be added to the queue
