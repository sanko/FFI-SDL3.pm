    ...
    attach rect => {
        ...
        SDL_EnclosePoints   => [
            ...
            'SDL_bool' => sub ( $inner, $_points, $count, $clip, $result ) {
                my $points = $SDL3::Point::LIST->create(
...
SDL3::rect - SDL3::Rect Management Functions
...
This package defines functions used to manage L<SDL3::Rect> structures they may
...
Find out if a given C<point> lies within a C<rectangle>.
    ...
    my $dot  = SDL3::Point->new( { x => 100, y => 100 } );
    my $box1 = SDL3::Rect->new( { x => 0,  y => 0, w => 50,  h => 50 } );
    my $box2 = SDL3::Rect->new( { x => 50, y => 0, w => 100, h => 101 } );
...
=item C<point> - an L<SDL3::Point> structure

=item C<rectangle> - an L<SDL3::Rect> structure
...
=head2 C<SDL_RectEmpty( ... )>
    ...
    my $box = SDL3::Rect->new( { w => 0, h => 100 } );
...
=item C<rectangle> - an L<SDL3::Rect> structure to query
...
dimensionally the same.
    ...
    my $box1 = SDL3::Rect->new( { w => 0, h => 100, x => 5 } );
    my $box2 = SDL3::Rect->new( { w => 0, h => 100, x => 10 } );
    my $box3 = SDL3::Rect->new( { w => 0, h => 100, x => 10 } );
...
=item C<lhs> - first L<SDL3::Rect> structure

=item C<rhs> - second L<SDL3::Rect> structure
...
Determine whether two rectangles intersect.
    ...
    my $box1 = SDL3::Rect->new( { w => 10, h => 100, x => 5,   y => 10 } );
    my $box2 = SDL3::Rect->new( { w => 10, h => 100, x => 10,  y => 0 } );
    my $box3 = SDL3::Rect->new( { w => 10, h => 100, x => 100, y => 0 } );
...
=item C<lhs> - an L<SDL3::Rect> structure representing the first rectangle

=item C<rhs> - an L<SDL3::Rect> structure representing the second rectangle
...
=item C<result> an L<SDL3::Rect> structure which will be filled in with the intersection of rectangles C<lhs> and C<rhs>
...
Calculate a minimal rectangle enclosing a set of points.
    ...
    my $p1  = SDL3::Point->new( { x => 1, y => 10 } );
    my $p2  = SDL3::Point->new( { x => 5, y => 20 } );
    my $res = SDL3::Rect->new();
...
=item C<points> -  an array of L<SDL3::Point> structures representing points to be enclosed

=item C<count> - the number of structures in the C<points> array

=item C<clip> - an L<SDL3::Rect> used for clipping or C<undef> to enclose all points

=item C<result> - an L<SDL3::Rect> structure filled in with the minimal enclosing rectangle
...
Calculate the intersection of a rectangle and line segment.
    ...
    my $box = SDL3::Rect->new( { x => 0, y => 0, w => 32, h => 32 } );
    my $x1  = 0;
    my $y1  = 0;
    my $x2  = 31;
    my $y2  = 31;
...
=item C<rect> - an L<SDL3::Rect> structure representing the rectangle to intersect
