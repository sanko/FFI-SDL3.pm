package SDL3::video {
    ...
    enum SDL_WindowFlags => [
        ...
        [ SDL_WINDOW_FULLSCREEN_DESKTOP => sub { ( SDL3::SDL_WINDOW_FULLSCREEN() | 0x00001000 ) } ],
        ...
        [ SDL_WINDOW_INPUT_GRABBED      => sub { SDL3::SDL_WINDOW_MOUSE_GRABBED() } ]
    ];
    define video => [
        [ SDL_WINDOWPOS_UNDEFINED_MASK => 0x1FFF0000 ],
        [   SDL_WINDOWPOS_UNDEFINED_DISPLAY =>
                sub ($X) { ( SDL3::SDL_WINDOWPOS_UNDEFINED_MASK() | ($X) ) }
        ],
        [ SDL_WINDOWPOS_UNDEFINED => sub () { SDL3::SDL_WINDOWPOS_UNDEFINED_DISPLAY(0) } ],
        [   SDL_WINDOWPOS_ISUNDEFINED => sub ($X) {
                ( ( ($X) & 0xFFFF0000 ) == SDL3::SDL_WINDOWPOS_UNDEFINED_MASK() )
            }
        ],
        [ SDL_WINDOWPOS_CENTERED_MASK => sub () {0x2FFF0000} ],
        [   SDL_WINDOWPOS_CENTERED_DISPLAY =>
                sub ($X) { ( SDL3::SDL_WINDOWPOS_CENTERED_MASK() | ($X) ) }
        ],
        [ SDL_WINDOWPOS_CENTERED => sub() { SDL3::SDL_WINDOWPOS_CENTERED_DISPLAY(0) } ],
        [   SDL_WINDOWPOS_ISCENTERED => sub ($X) {
                ( ( ($X) & 0xFFFF0000 ) == SDL3::SDL_WINDOWPOS_CENTERED_MASK() )
            }
        ],
    ];
    ...
    package SDL3::GLContext 0.01 {
        use SDL3::Utils qw[has];
        our $TYPE = has();    # opaque
    };
    enum SDL_GLattr => [
        qw[
...
SDL3::video - SDL Video Functions
...
SDL3::version represents the library's version as three levels: major, minor,
and patch level.
...
=item C<rect> - the L<SDL3::Rect> structure filled in with the display bounds
...
=item C<mode> - an L<SDL3::DisplayMode> structure filled in with the mode at C<modeIndex>
...
=item C<mode> - an L<SDL3::DisplayMode> structure filled in with the current display mode
...
=item C<mode> - an L<SDL3::DisplayMode> structure containing the desired display mode

=item C<closest> - an L<SDL3::DisplayMode> structure filled in with the closest match of the available display modes
...
=item C<mode> - the L<SDL3::DisplayMode> structure representing the mode to use, or C<undef> to use the window's dimensions and the desktop's format and refresh rate
...
=item C<mode> - an L<SDL3::DisplayMode> structure filled in with the fullscreen display mode
...
The numeric ID is what L<SDL3::WindowEvent> references, and is necessary to map
these events to specific L<SDL3::Window> objects.
...
=item C<icon> - an L<SDL3::Surface> structure containing the icon for the window
...
=item C<rects> - an array of L<SDL3::Rect> structures representing areas of the surface to copy
...
=item C<win> - the L<SDL3::Window> where hit-testing was set on

=item C<area> - an L<SDL3::Point> which should be hit-tested
