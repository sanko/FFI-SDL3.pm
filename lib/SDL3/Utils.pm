package SDL3::Utils {
    ...
    #sub ver() {    # TODO: Remove in favor of SDL3::version
    #    CORE::state $ver;
    #    $ver // ffi()->function( SDL_GetVersion => ['SDL_Version'] )
    #        ->call( $ver = SDL3::Version->new() );
    #    $ver;
    #}
    ...
    sub ffi () {
        CORE::state $ffi;
        if ( !defined $ffi ) {
            my $distdir = Path::Tiny->new( dist_dir('SDL3') );
    ...
    sub enumX {
        ...
        ffi->load_custom_type( 'SDL3::Utils::Type::Enum', $name, \%config, @values );
    ...
    sub enum (%args) {
        ...
        $package = 'SDL3' unless
            ...
            # Known knowns
            $package eq 'SDL3::Image' ||
            $package eq 'SDL3::TTF'   ||
            $package eq 'SDL3::Mixer' ||
            $package eq 'SDL3::GFX';
        #
...
#ffi->load_custom_type('SDL3::Utils::Type::Enum', $tag => { rev=>'int', package => $package, maps => \@maps } => $args{$tag} );
            enumX(
                ffi,
                $tag => $args{$tag},
