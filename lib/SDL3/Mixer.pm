package SDL3::Mixer 0.01 {
    ...
    #
    define version => [
        [ SDL_MIXER_MAJOR_VERSION => sub () { SDL3::Mixer::_ver()->major } ],
        [ SDL_MIXER_MINOR_VERSION => sub () { SDL3::Mixer::_ver()->minor } ],
        [ SDL_MIXER_PATCHLEVEL    => sub () { SDL3::Mixer::_ver()->patch } ],
        ...
        [   SDL_MIXER_COMPILEDVERSION => sub () {
                SDL3::SDL_VERSIONNUM( SDL_MIXER_MAJOR_VERSION(), SDL_MIXER_MINOR_VERSION(),
                    SDL_MIXER_PATCHLEVEL() );
            }
        ],
        [   SDL_MIXER_VERSION_ATLEAST => sub ( $X, $Y, $Z ) {
                ( SDL_MIXER_COMPILEDVERSION() >= SDL3::SDL_VERSIONNUM( $X, $Y, $Z ) )
            }
        ]
    ];
    ...
    define default => [
        ...
        [   MIX_DEFAULT_FORMAT => SDL3::SDL_BYTEORDER() eq SDL3::SDL_LIL_ENDIAN() ?
                SDL3::AUDIO_S16MSB() :
                SDL3::AUDIO_S16LSB()
        ],
    ...
    package SDL3::Mixer::Chunk {
        use strict;
        use warnings;
        use SDL3::Utils qw[has ffi];
        our $TYPE = has
            allocated => 'int',
            _abuf     => 'opaque',         # uint8*
            alen      => 'uint32',
...
=cut
    ...
    define image => [
        [ Mix_SetError => sub (@args) { SDL3::SDL_SetError(@args) } ],
        [ Mix_GetError => sub (@args) { SDL3::SDL_GetError(@args) } ],
    ];
...
SDL3::Mixer wraps the C<SDL_mixer> library, a simple multi-channel audio mixer.
...
This can load WAVE, AIFF, RIFF, OGG, and VOC formats. Using L<SDL3::RWops> is not covered
...
=item C<src> - the source L<SDL3::RWops> to load the sample from

=item C<freesrc> - a non-zero value means we will automatically close/free the C<src> for you

=back

Returns a pointer to the sample as a L<SDL3::Mixer::Chunk> object. C<undef> is returned on errors.
...
Returns a pointer to the sample as a L<SDL3::Mixer::Music> object. C<undef> is returned on errors.

=head2 C<Mix_LoadMUS_RW( ... )>

Load a music C<src> from an L<SDL3::RWops> object.
...
=item C<src> - the source L<SDL3::RWops> to load the music from

=item C<freesrc> - a non-zero value means we will automatically close/free the C<src> for you

=back

Returns a pointer to the music as a L<SDL3::Mixer::Music> object. C<undef> is returned on errors.

=head2 C<Mix_LoadMUSType_RW( ... )>

Load a music C<src> from an L<SDL3::RWops> object assuming a specific format.
...
=item C<chunk> - pointer to the L<SDL3::Mixer::Chunk> to free
...
=item C<music> - pointer to the L<SDL3::Mixer::Music> to free
...
=item C<music> - L<SDL3::Mixer::Music> structure to query
