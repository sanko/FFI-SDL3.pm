                ...
                do {
                    if ( ref $code eq 'CODE' ) {
                        while ( !eval { &{$code}; } ) {
                            ...
                            CORE::state $sdl_assert_data //= SDL3::AssertData->new(
                                {   condition =>

                                        #SDL3::Utils::ffi->cast( 'string', 'opaque', $condition ),
                                        $condition
                                }
                            );
                            my $sdl_assert_state
...
SDL3::assert - SDL Assertion Functions
...
SDL3::assert implements an assertion system. Failures even cause the perl
...
=item C<data> - a pointer to the L<SDL3::AssertData> structure corresponding to the current assertion
...
Defines and Enumerations listed here may be imported from SDL3 with the
following tags:
