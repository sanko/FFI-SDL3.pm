...
SDL3::hints - SDL Configuration Variables
...
Minimize your L<SDL3::Window> if it loses key focus when in fullscreen mode.
Defaults to false.
...
then two things will occur on the newly created L<SDL3::Window>:

=over

=item 1. Its pixel format will be set to the same pixel format as this L<SDL3::Window>. This is needed for example when sharing an OpenGL context across multiple windows.
...
=item The address (as a string C<%p>) of the L<SDL3::Window> that new windows created with C<SDL_CreateWindowFrom( ... )> should share a pixel format with
