package SDL3::version 0.01 {
    ...
    sub _ver() {
        CORE::state $version;
        if ( !defined $version ) {
            $version = SDL3::Version->new();
            SDL3::SDL_VERSION($version);
        }
        $version;
    }
    define version => [
        [ SDL_MAJOR_VERSION => sub () { SDL3::version::_ver()->major } ],
        [ SDL_MINOR_VERSION => sub () { SDL3::version::_ver()->minor } ],
        [ SDL_PATCHLEVEL    => sub () { SDL3::version::_ver()->patch } ],
        [ SDL_VERSION       => sub ($version) { SDL3::SDL_GetVersion($version); } ],
        [ SDL_VERSIONNUM    => sub ( $X, $Y, $Z ) { ( ($X) * 1000 + ($Y) * 100 + ($Z) ) } ],
        [   SDL_COMPILEDVERSION => sub () {
                SDL3::SDL_VERSIONNUM( SDL3::SDL_MAJOR_VERSION(), SDL3::SDL_MINOR_VERSION(),
                    SDL3::SDL_PATCHLEVEL() );
            }
        ],
        [   SDL_VERSION_ATLEAST => sub ( $X, $Y, $Z ) {
                ( SDL3::SDL_COMPILEDVERSION() >= SDL3::SDL_VERSIONNUM( $X, $Y, $Z ) )
            }
        ]
    ];
    attach version =>
...
SDL3::version - Information About the Version of SDL in Use
...
Get the version of SDL that is linked against your program.
	...
	SDL_GetVersion( my $version_ = SDL3::Version->new() );
...
=item C<ver> - the L<SDL3::Version> structure that contains the version information
