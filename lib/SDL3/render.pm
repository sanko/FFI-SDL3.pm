package SDL3::render 0.01 {
    ...
    package SDL3::Renderer {
        use SDL3::Utils;
        our $TYPE = has();
    };

    package SDL3::Texture {
        use SDL3::Utils;
        our $TYPE = has();
    };
    attach render => {
        ...
        SDL_CreateWindowAndRenderer => [
            ...
            'int' => sub {
                my ( $inner, $width, $height, $window_flags, $window, $renderer ) = @_;

                #$window   //= SDL3::Window->new;
                #$renderer //= SDL3::Renderer->new;
...
SDL3::render - SDL 2D Rendering Functions
...
Returns an L<SDL3::RendererInfo> structure on success or a negative error code
...
Returns a L<SDL3::Window> and L<SDL3::Renderer> objects on success, or C<-1> on
...
used with an L<SDL3::Window> as the final destination and not an
L<SDL3::Surface>.
...
=item C<surface> - the L<SDL3::Surface> structure representing the surface where rendering is done
...
=item C<surface> - the L<SDL3::Surface> structure containing pixel data used to fill the texture
...
Update the given texture rectangle with new pixel data.
	...
	my $rect = SDL3::Rect->new( { x => 0, y => ..., w => $surface->w, h => $surface->h } );
...
=item C<rect> - an L<SDL3::Rect> structure representing the area to update, or undef to update the entire texture
...
=item C<rect> - an L<SDL3::Rect> structure representing the area to lock for access; undef to lock the entire texture
...
Besides providing an L<SDL3::Surface> instead of raw pixel data, this function
operates like L<SDL3::LockTexture>.
...
Returns the L<SDL3::Surface> structure on success, or C<-1> if the texture is
...
bit in the flags of L<SDL3::RendererInfo> to see if render targets are
supported.
...
This may return C<0> for C<w> and C<h> if the L<SDL3::Renderer> has never had
...
=item C<rect> - the L<SDL3::Rect> structure representing the drawing area, or undef to set the viewport to the entire target
...
Returns an L<SDL3::Rect> structure filled in with the current drawing area.
...
=item C<rect> - an L<SDL3::Rect> structure representing the clip area, relative to the viewport, or undef to disable clipping
...
Returns an L<SDL3::Rect> structure filled in with the current clipping area or
...
Draw multiple points on the current rendering target.
	...
	my @points = map { SDL3::Point->new( {x => int rand, y => int rand } ) } 1..1024;
...
=item C<points> - an array of L<SDL3::Point> structures that represent the points to draw
...
=item C<points> - an array of L<SDL3::Point> structures representing points along the lines
...
Draw a rectangle on the current rendering target.
	...
	SDL_RenderDrawRect( $renderer, SDL3::Rect->new( { x => 100, y => 100, w => 100, h => 100 } ) );
...
=item C<rect> - an L<SDL3::Rect> structure representing the rectangle to draw
...
Draw some number of rectangles on the current rendering target.
	...
	SDL_RenderDrawRects( $renderer,
		SDL3::Rect->new( { x => 100, y => 100, w => 100, h => 100 } ),
        SDL3::Rect->new( { x => 75,  y => 75,  w => 50,  h => 50 } )
    );
...
=item C<rects> - an array of L<SDL3::Rect> structures representing the rectangles to be drawn
...
Fill a rectangle on the current rendering target with the drawing color.
	...
	SDL_RenderFillRect( $renderer, SDL3::Rect->new( { x => 100, y => 100, w => 100, h => 100 } ) );
...
=item C<rect> - the L<SDL3::Rect> structure representing the rectangle to fill
...
=item C<rects> - an array of L<SDL3::Rect> structures representing the rectangles to be filled
...
=item C<srcrect> - the source L<SDL3::Rect> structure

=for TODO: or NULL for the entire texture

=item C<dstrect> - the destination L<SDL3::Rect> structure; the texture will be stretched to fill the given rectangle
...
Read pixels from the current rendering target to an array of pixels.
	...
	SDL_RenderReadPixels(
        $renderer,
        SDL3::Rect->new( { x => 0, y => 0, w => 640, h => 480 } ),
...
=item C<rect> - an L<SDL3::Rect> structure representing the area to read
...
You need a renderer to create an L<SDL3::Texture>, therefore you can only use
