package SDL3::stdinc 0.01 {
    ...
    define stdinc => [
        [ SDL_zero => sub ($ptr) { SDL3::SDL_memset( $ptr, 0, length($ptr) ) } ],
        [   SDL_sscanf => sub {    # FIX: Breaks on string captures
                my ( $buffer, $format, @args ) = @_;
                my @types = SDL3::Utils::_tokenize_in($format);
                my $retval
        ...
        [ SDL_vsscanf => sub { SDL3::SDL_sscanf(@_) } ],
        [   SDL_snprintf => sub {
                my ( $buffer, $maxlen, $format, @args ) = @_;
                $buffer = "\0" x $maxlen;      # Just to be safe since we know $maxlen
                my @types = SDL3::Utils::_tokenize_out( $format, 0 );
        ...
        [ SDL_vsnprintf => sub { SDL3::SDL_snprintf(@_) } ],
        ...
        [   SDL_iconv_utf8_locale => sub ($s) {
                SDL3::SDL_iconv_string( "", "UTF-8", $s, SDL3::SDL_strlen($s) + 1 );
            }
        ],

#[SDL_iconv_utf8_ucs2   => sub ($s) { SDL3::SDL_iconv_string("UCS-2-INTERNAL", "UTF-8", $s, SDL3::SDL_strlen($s)+1 )}],
#[SDL_iconv_utf8_ucs4   => sub ($s) { SDL3::SDL_iconv_string("UCS-4-INTERNAL", "UTF-8", $s, SDL3::SDL_strlen($s)+1 )}]
    ];
...
SDL3::stdinc - General C Language Support Functions
...
Returns a new L<SDL3::iconv_t> object.
...
=item C<cd> - L<SDL3::iconv_t> object
