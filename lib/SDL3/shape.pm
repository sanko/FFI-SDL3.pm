package SDL3::shape 0.01 {
    ...
    define shape => [
        [   SDL_SHAPEMODEALPHA => sub ($mode) {
                ( $mode == SDL3::ShapeModeDefault() ||
                        $mode == SDL3::ShapeModeBinarizeAlpha() ||
                        $mode == SDL3::ShapeModeReverseBinarizeAlpha() )
            }
        ],
    ];

    package SDL3::WindowShapeParams {
        use SDL3::Utils;
        use FFI::C::UnionDef;
        is 'Union';
...
SDL3::shape - Functions for the Shaped Window API
...
SDL3::shape exposes functions which allow you to create windows with custom
shapes.
...
argument, or C<SDL_NONSHAPEABLE_WINDOW> if the L<SDL3::Window> given does not
...
C<SDL_NONSHAPEABLE_WINDOW> if the L<SDL3::Window> given is not a shaped window,
or C<SDL_WINDOW_LACKS_SHAPE> if the L<SDL3::Window> given is a shapeable window
