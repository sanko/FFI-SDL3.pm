package SDL3::surface 0.01 {
    ...
    attach surface => {
        ...
        SDL_FillRects => [
            ...
            'int' => sub ( $inner, $dst, $_rects, $count, $color ) {
                my $rects = $SDL3::Rect::LIST->create(
    ...
    define surface => [
        [   SDL_LoadBMP => sub ($file) {
                SDL3::SDL_LoadBMP_RW( SDL3::SDL_RWFromFile( $file, 'rb' ), 1 );
            }
        ],
        [   SDL_SaveBMP => sub ( $surface, $file ) {
                SDL3::SDL_SaveBMP_RW( $surface, SDL3::SDL_RWFromFile( $file, 'wb' ), 1 );
            }
        ],
        [   SDL_BlitSurface => sub ( $src, $srcrect, $dst, $dstrect ) {
                SDL3::SDL_UpperBlit( $src, $srcrect, $dst, $dstrect );
            }
        ],
        [   SDL_BlitScaled => sub ( $src, $srcrect, $dst, $dstrect ) {
                SDL3::SDL_UpperBlitScaled( $src, $srcrect, $dst, $dstrect );
            }
        ],
    ];
...
SDL3::surface - SDL3::Surface Management Functions
...
SDL3::surface contains enumerations, functions, and other values used to manage
a SDL3::Surface structure.
...
Returns the new L<SDL3::Surface> structure that is created or C<undef> if it
...
Allocate a new RGB surface with existing pixel data.
	...
	# Declare an SDL3::Surface to be filled with raw pixel data
    my $pixels = [
...
Returns the new L<SDL3::Surface> structure that is created or NULL if it fails;
...
=item C<surface> - the C<SDL3::Surface> to free
...
=item C<surface> - the L<SDL3::Surface> structure to update

=item C<palette> - the L<SDL3::Palette> structure to use
...
=item C<surface> - the L<SDL3::Surface> structure to be locked
...
=item C<surface> - the L<SDL3::Surface> structure to be unlocked
...
Returns a pointer to a new L<SDL3::Surface> structure or C<undef> if there was
...
=item C<surface> - the L<SDL3::Surface> structure containing the image to be saved
...
=item C<surface> - the L<SDL3::Surface> structure to optimize
...
=item C<surface> the L<SDL3::Surface> structure to query
...
=item C<surface> - the L<SDL3::Surface> structure to be clipped
...
=item C<surface> - the L<SDL3::Surface> structure representing the surface to be clipped
...
=item C<src> - the existing L<SDL3::Surface> structure to convert
...
=item C<dst> - the L<SDL3::Surface> structure that is the drawing target
...
Perform a fast fill of a set of rectangles with a specific color.
    ...
    my @rects = (
        SDL3::Rect->new( { w => 100, h => 100, x => 0,   y => 0 } ),
        SDL3::Rect->new( { w => 100, h => 100, x => 200, y => 20 } )
    );
...
=item C<rects> - an array of C<SDL3::Rects> representing the rectangles to fill.
...
=item C<src> - the C<SDL3::Surface> structure to be copied from

=item C<srcrect> - the L<SDL3::Rect> structure representing the rectangle to be copied, or C<undef> to copy the entire surface

=item C<dst> - the L<SDL3::Surface> structure that is the blit target

=item C<dstrect> - the L<SDL3::Rect> structure representing the rectangle that is copied into
...
=item C<src> - the L<SDL3::Surface> structure to be copied from
