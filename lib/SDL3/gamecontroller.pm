package SDL3::gamecontroller 0.01 {
    use SDL3::Utils;
    ...
    package SDL3::GameControllerButtonBind {
        use SDL3::Utils;

        package SDL3::GameControllerButtonBind_Hat {
            use SDL3::Utils;
            our $TYPE = has
                hat      => 'int',
                hat_mask => 'int';
        ...
        package SDL3::GameControllerButtonBind_Value {
            use SDL3::Utils;
            is 'Union';
            our $TYPE = has
                button => 'int',
    ...
    define gamecontroller => [
        [   SDL_GameControllerAddMappingsFromFile => sub ($file) {
                SDL3::SDL_GameControllerAddMappingsFromRW( SDL3::SDL_RWFromFile( $file, 'rb' ), 1 );
            }
        ]
    ];
...
SDL3::gamecontroller - SDL Game Controller Event Handling

=head1 SYNOPSIS

    use SDL3 qw[:gamecontroller];
...
C<SDL_JoystickOpen( ... )>|SDL3::joystick/C<SDL_JoystickOpen( ... )> >>.

Expected parameters include:

=over

=item C<joystick_index> - the C<device_index> of a device, up to L<< C<SDL_NumJoysticks( )>|SDL3::joystick/C<SDL_NumJoysticks( )> >>
...
=item C<joystick_index> - the device_index of a device, up to L<< C<SDL_NumJoysticks( )>|SDL3::joystick/C<SDL_NumJoysticks( )> >>
...
=item C<joyid> - the instance id to get the C<SDL3::GameController> for

=back

Returns an L<SDL3::GameController> on success or C<undef> on failure; call
C<SDL_GetError( )> for more information.

=head2 C<SDL_GameControllerFromPlayerIndex( ... )>

Get the L<SDL3::GameController> associated with a player index.
...
Returns the L<SDL3::GameController> associated with a player index.
...
This function will give you a L<SDL3::Joystick> object, which allows you to use
the L<SDL3::Joystick> functions with a L<SDL3::GameController> object. This
would be useful for getting a joystick's position at any given time, even if it
hasn't moved (moving it would produce an event, which would have the axis'
value).

The pointer returned is owned by the L<SDL3::GameController>. You should not
call L<< C<SDL_JoystickClose( ... )>|SDL3::joystick/C<SDL_JoystickClose( ... )>
...
Returns a L<SDL3::Joystick> object; call C<SDL_GetError( )> for more
information.
...
This function is called internally to translate L<SDL3::GameController> mapping
strings for the underlying joystick device into the consistent
L<SDL3::GameController> mapping. You do not normally need to call this function
unless you are parsing L<SDL3::GameController> mappings in your own code.

Expected parameters include:

=over

=item C<str> - string representing a L<SDL3::GameController> axis
...
specified. The string returned is of the format used by L<SDL3::GameController>
mapping strings.
...
Returns a L<SDL3::GameControllerButtonBind> describing the bind. On failure
...
=item C<button> - an enum value for a given L<SDL3::GameControllerButton>
