package SDL3::audio {
    ...
    define audio => [
        ...
        [ SDL_AUDIO_BITSIZE        => sub ($x) { $x & SDL3::SDL_AUDIO_MASK_BITSIZE() } ],
        [ SDL_AUDIO_ISFLOAT        => sub ($x) { $x & SDL3::SDL_AUDIO_MASK_DATATYPE() } ],
        [ SDL_AUDIO_ISBIGENDIAN    => sub ($x) { $x & SDL3::SDL_AUDIO_MASK_ENDIAN() } ],
        [ SDL_AUDIO_ISSIGNED       => sub ($x) { $x & SDL3::SDL_AUDIO_MASK_SIGNED() } ],
        [ SDL_AUDIO_ISINT          => sub ($x) { !SDL3::DL_AUDIO_ISFLOAT($x) } ],
        [ SDL_AUDIO_ISLITTLEENDIAN => sub ($x) { !SDL3::SDL_AUDIO_ISBIGENDIAN($x) } ],
        [ SDL_AUDIO_ISUNSIGNED     => sub ($x) { !SDL3::SDL_AUDIO_ISSIGNED($x) } ],

        # audio format flags
        [ AUDIO_U8     => 0x0008 ], [ AUDIO_S8     => 0x8008 ], [ AUDIO_U16LSB => 0x0010 ],
        [ AUDIO_S16LSB => 0x8010 ], [ AUDIO_U16MSB => 0x1010 ], [ AUDIO_S16MSB => 0x9010 ],
        [ AUDIO_U16    => sub () { SDL3::AUDIO_U16LSB() } ],
        [ AUDIO_S16    => sub () { SDL3::AUDIO_S16LSB() } ],

        # int32 support
        [ AUDIO_S32LSB => 0x8020 ], [ AUDIO_S32MSB => 0x9020 ],
        [ AUDIO_S32    => sub () { SDL3::AUDIO_S32LSB() } ],

        # float32 support
        [ AUDIO_F32LSB => 0x8120 ], [ AUDIO_F32MSB => 0x9120 ],
        [ AUDIO_F32    => sub () { SDL3::AUDIO_F32LSB() } ],

        # Native audio byte ordering
        (
            SDL3::SDL_BYTEORDER() == SDL3::SDL_BIG_ENDIAN() ? (
                [ AUDIO_U16SYS => sub () { SDL3::AUDIO_U16MSB() } ],
                [ AUDIO_S16SYS => sub () { SDL3::AUDIO_S16MSB() } ],
                [ AUDIO_S32SYS => sub () { SDL3::AUDIO_S32MSB() } ],
                [ AUDIO_F32SYS => sub () { SDL3::AUDIO_F32MSB() } ]
                ) : (
                [ AUDIO_U16SYS => sub () { SDL3::AUDIO_U16LSB() } ],
                [ AUDIO_S16SYS => sub () { SDL3::AUDIO_S16LSB() } ],
                [ AUDIO_S32SYS => sub () { SDL3::AUDIO_S32LSB() } ],
                [ AUDIO_F32SYS => sub () { SDL3::AUDIO_F32LSB() } ],
                )
        ),
        ...
        [   SDL_AUDIO_ALLOW_ANY_CHANGE => sub () {
                ( SDL3::SDL_AUDIO_ALLOW_FREQUENCY_CHANGE()
                        | SDL3::SDL_AUDIO_ALLOW_FORMAT_CHANGE()
                        | SDL3::SDL_AUDIO_ALLOW_CHANNELS_CHANGE()
                        | SDL3::SDL_AUDIO_ALLOW_SAMPLES_CHANGE() )
            }
        ]
    ];
...
END
    ...
    define audio => [
        [   SDL_LoadWAV => sub ( $file, $spec, $audio_buf, $audio_len ) {
                SDL3::SDL_LoadWAV_RW( SDL3::SDL_RWFromFile( $file, 'rb' ),
...
SDL3::audio - SDL Audio Functions
...
=item C<desired> - an L<SDL3::AudioSpec> structure representing the desired output format. Please refer to the C<SDL_OpenAudioDevice( ... )> documentation for details on how to prepare this structure.

=item C<obtained> - an L<SDL3::AudioSpec> structure filled in with the actual parameters, or NULL.
...
=item C<spec> - The L<SDL3::AudioSpec> to be initialized by this function.
...
=item C<desired> - an L<SDL3::AudioSpec> structure representing the desired output format; see C<SDL_OpenAudio( )> for more information

=item C<obtained> - an L<SDL3::AudioSpec> structure filled in with the actual output format; see C<SDL_OpenAudio( )> for more information
...
C<audio_buf> and its length in bytes to C<audio_len>. The L<SDL3::AudioSpec>
...
=item C<cvt> an L<SDL3::AudioCVT> structure that was previously set up by C<SDL_BuildAudioCVT( ... )>.
...
Returns a L<SDL3::AudioStream> on success.
...
=item C<userdata> - an application-specific parameter saved in the L<SDL3::AudioSpec> structure
...
=item C<cvt> - L<SDL3::AudioCVT> structure
...
The maximum number of C<SDL_AudioFilter> functions in L<SDL3::AudioCVT> is
currently limited to 9. The L<SDL3::AudioCVT>->filters array has 10 pointers,
